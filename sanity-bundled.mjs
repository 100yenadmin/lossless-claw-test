...
function probeVirtualTable(db2, sql) {
  try {
    db2.exec("DROP TABLE IF EXISTS temp.__lcm_virtual_table_probe");
    db2.exec(sql);
    db2.exec("DROP TABLE temp.__lcm_virtual_table_probe");
    return true;
  } catch {
    try {
      db2.exec("DROP TABLE IF EXISTS temp.__lcm_virtual_table_probe");
...
function probeFts5(db2) {
  return probeVirtualTable(
    db2,
    "CREATE VIRTUAL TABLE temp.__lcm_virtual_table_probe USING fts5(content)"
  );
}
function probeTrigramTokenizer(db2) {
  return probeVirtualTable(
    db2,
    "CREATE VIRTUAL TABLE temp.__lcm_virtual_table_probe USING fts5(content, tokenize='trigram')"
  );
}
function getLcmDbFeatures(db2) {
  const cached = featureCache.get(db2);
  if (cached) {
    return cached;
  }
  const detected = {
    fts5Available: probeFts5(db2),
    trigramTokenizerAvailable: false
  };
  if (detected.fts5Available) {
    detected.trigramTokenizerAvailable = probeTrigramTokenizer(db2);
  }
  featureCache.set(db2, detected);
  return detected;
}

// src/store/message-identity.ts
import { createHash } from "node:crypto";
function buildMessageIdentityHash(role, content) {
...
function parseUtcTimestamp(value) {
  ...
  return /* @__PURE__ */ new Date(`${normalized}Z`);
}
function parseUtcTimestampOrNull(value) {
  if (value == null) return null;
  return parseUtcTimestamp(value);
}

// src/db/migration.ts
var VERSIONED_BACKFILL_STEPS = {
  backfillSummaryDepths: 1,
  backfillSummaryMetadata: 1,
  backfillToolCallColumns: 1
};
function ensureSummaryDepthColumn(db2) {
  const summaryColumns = db2.prepare(`PRAGMA table_info(summaries)`).all();
  const hasDepth = summaryColumns.some((col) => col.name === "depth");
  if (!hasDepth) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN depth INTEGER NOT NULL DEFAULT 0`);
  }
}
function ensureSummaryMetadataColumns(db2) {
  const summaryColumns = db2.prepare(`PRAGMA table_info(summaries)`).all();
  ...
  if (!hasEarliestAt) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN earliest_at TEXT`);
  }
  if (!hasLatestAt) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN latest_at TEXT`);
  }
  if (!hasDescendantCount) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN descendant_count INTEGER NOT NULL DEFAULT 0`);
  }
  if (!hasDescendantTokenCount) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN descendant_token_count INTEGER NOT NULL DEFAULT 0`);
  }
  if (!hasSourceMessageTokenCount) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN source_message_token_count INTEGER NOT NULL DEFAULT 0`);
  }
}
...
function ensureSummaryModelColumn(db2) {
  const summaryColumns = db2.prepare(`PRAGMA table_info(summaries)`).all();
  const hasModel = summaryColumns.some((col) => col.name === "model");
  if (!hasModel) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN model TEXT NOT NULL DEFAULT 'unknown'`);
  }
}
function ensureSummaryV41Columns(db2) {
  const cols = db2.prepare(`PRAGMA table_info(summaries)`).all();
  const has = (name) => cols.some((c) => c.name === name);
  if (!has("session_key")) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN session_key TEXT NOT NULL DEFAULT ''`);
  }
  if (!has("suppressed_at")) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN suppressed_at TEXT`);
  }
  if (!has("entity_index")) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN entity_index TEXT`);
  }
  ...
  if (!has("suppress_reason")) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN suppress_reason TEXT`);
  }
  ...
  if (!has("leaf_summarizer_cap_was")) {
    db2.exec(`ALTER TABLE summaries ADD COLUMN leaf_summarizer_cap_was INTEGER`);
  }
}
function ensureMessageSuppressedAtColumn(db2) {
  const cols = db2.prepare(`PRAGMA table_info(messages)`).all();
  const hasSuppressedAt = cols.some((c) => c.name === "suppressed_at");
  if (!hasSuppressedAt) {
    db2.exec(`ALTER TABLE messages ADD COLUMN suppressed_at TEXT`);
  }
}
function ensureLcmFeatureFlagsTable(db2) {
  db2.exec(`
...
function ensureCompactionTelemetryColumns(db2) {
  const telemetryColumns = db2.prepare(`PRAGMA table_info(conversation_compaction_telemetry)`).all();
  ...
  if (!hasLastLeafCompactionAt) {
    db2.exec(`ALTER TABLE conversation_compaction_telemetry ADD COLUMN last_leaf_compaction_at TEXT`);
  }
  ...
  if (!hasLastApiCallAt) {
    db2.exec(`ALTER TABLE conversation_compaction_telemetry ADD COLUMN last_api_call_at TEXT`);
  }
  if (!hasLastCacheTouchAt) {
    db2.exec(`ALTER TABLE conversation_compaction_telemetry ADD COLUMN last_cache_touch_at TEXT`);
  }
  if (!hasProvider) {
    db2.exec(`ALTER TABLE conversation_compaction_telemetry ADD COLUMN provider TEXT`);
  }
  if (!hasModel) {
    db2.exec(`ALTER TABLE conversation_compaction_telemetry ADD COLUMN model TEXT`);
  }
  if (!hasLastObservedPromptTokenCount) {
    db2.exec(`ALTER TABLE conversation_compaction_telemetry ADD COLUMN last_observed_prompt_token_count INTEGER`);
  }
}
function ensureMessagePartsTable(db2) {
  const tables = db2.prepare(`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'message_parts'`).all();
...
function ensureMessageIdentityHashColumn(db2) {
  const messageColumns = db2.prepare(`PRAGMA table_info(messages)`).all();
  const hasIdentityHash = messageColumns.some((col) => col.name === "identity_hash");
  if (!hasIdentityHash) {
    db2.exec(`ALTER TABLE messages ADD COLUMN identity_hash TEXT`);
  }
}
function backfillMessageIdentityHashes(db2, options) {
  const selectStmt = db2.prepare(
  ...
  const updateStmt = db2.prepare(`UPDATE messages SET identity_hash = ? WHERE message_id = ?`);
  let lastProcessedMessageId = 0;
  const managesOwnTransaction = options?.managesOwnTransaction ?? true;
  while (true) {
    const rows = selectStmt.all(lastProcessedMessageId, 1e3);
    if (rows.length === 0) {
      return;
    }
...
function hasCompletedVersionedBackfill(db2, stepName, algorithmVersion) {
...
function markVersionedBackfillComplete(db2, stepName, algorithmVersion) {
  db2.prepare(
...
function rollbackSavepoint(db2, savepointName) {
  try {
    db2.exec(`ROLLBACK TO SAVEPOINT ${savepointName}`);
  } finally {
    db2.exec(`RELEASE SAVEPOINT ${savepointName}`);
  }
}
function runVersionedBackfillStep(db2, stepName, log, step) {
  const algorithmVersion = VERSIONED_BACKFILL_STEPS[stepName];
  if (hasCompletedVersionedBackfill(db2, stepName, algorithmVersion)) {
    log?.info?.(
  ...
  db2.exec(`SAVEPOINT ${savepointName}`);
  try {
    step();
    markVersionedBackfillComplete(db2, stepName, algorithmVersion);
    db2.exec(`RELEASE SAVEPOINT ${savepointName}`);
    log?.info?.(
      `[lcm] migration step complete: step=${stepName} algorithmVersion=${algorithmVersion} durationMs=${Date.now() - startedAt}`
    );
  } catch (error) {
    rollbackSavepoint(db2, savepointName);
    log?.info?.(
...
function backfillSummaryDepths(db2) {
  db2.exec(`UPDATE summaries SET depth = 0 WHERE kind = 'leaf'`);
  const conversationRows = db2.prepare(`SELECT DISTINCT conversation_id FROM summaries WHERE kind = 'condensed'`).all();
  if (conversationRows.length === 0) {
    return;
  }
  const updateDepthStmt = db2.prepare(`UPDATE summaries SET depth = ? WHERE summary_id = ?`);
  for (const row of conversationRows) {
    const conversationId = row.conversation_id;
    const summaries = db2.prepare(
      `SELECT summary_id, conversation_id, kind, depth, token_count, created_at
         FROM summaries
         WHERE conversation_id = ?`
    ).all(conversationId);
    const depthBySummaryId = /* @__PURE__ */ new Map();
    const unresolvedCondensedIds = /* @__PURE__ */ new Set();
    ...
    const parentsBySummaryId = /* @__PURE__ */ new Map();
...
function backfillSummaryMetadata(db2) {
  const conversationRows = db2.prepare(`SELECT DISTINCT conversation_id FROM summaries`).all();
  if (conversationRows.length === 0) {
    return;
  }
  const updateMetadataStmt = db2.prepare(
    `UPDATE summaries
  ...
  for (const conversationRow of conversationRows) {
    ...
    const metadataBySummaryId = /* @__PURE__ */ new Map();
    ...
    for (const summary of summaries) {
      ...
      if (summary.kind === "leaf") {
        ...
        const earliestAt2 = parseTimestamp(range?.earliestAt ?? summary.created_at) ?? fallbackDate;
        const latestAt2 = parseTimestamp(range?.latestAt ?? summary.created_at) ?? fallbackDate;
        metadataBySummaryId.set(summary.summary_id, {
          earliestAt: earliestAt2,
          latestAt: latestAt2,
          descendantCount: 0,
      ...
      for (const parentId of parentIds) {
        ...
        descendantTokenCount += Math.max(0, parentTokenCount) + Math.max(0, parentMetadata.descendantTokenCount);
...
function backfillToolCallColumns(db2) {
  db2.exec(
...
function getExistingTableNames(db2, names) {
  if (names.length === 0) {
    return /* @__PURE__ */ new Set();
  }
  const placeholders = names.map(() => "?").join(", ");
  const rows = db2.prepare(`SELECT name FROM sqlite_master WHERE type = 'table' AND name IN (${placeholders})`).all(...names);
  return new Set(
    rows.map((row) => row.name).filter((name) => typeof name === "string" && name.length > 0)
  );
}
...
function shouldRecreateStandaloneFtsTable(db2, spec) {
  const shadowTables = getFtsShadowTableNames(spec.tableName);
  const existingTables = getExistingTableNames(db2, [spec.tableName, ...shadowTables]);
  ...
  try {
    const info = db2.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name = ?").get(spec.tableName);
    const sql = info?.sql ?? "";
    if (spec.staleSchemaPatterns?.some((pattern) => sql.includes(pattern))) {
      return true;
    }
    const columns = db2.prepare(`PRAGMA table_info(${quoteSqlIdentifier(spec.tableName)})`).all();
    const columnNames = new Set(
      columns.map((col) => col.name).filter((name) => typeof name === "string" && name.length > 0)
    );
...
function ensureStandaloneFtsTable(db2, spec) {
  if (!shouldRecreateStandaloneFtsTable(db2, spec)) {
    return;
  }
  db2.exec(`DROP TABLE IF EXISTS ${quoteSqlIdentifier(spec.tableName)}`);
  for (const shadowTableName of getFtsShadowTableNames(spec.tableName)) {
    db2.exec(`DROP TABLE IF EXISTS ${quoteSqlIdentifier(shadowTableName)}`);
  }
  db2.exec(spec.createSql);
  db2.exec(spec.seedSql);
}
function runLcmMigrations(db2, options) {
  ...
  try {
    ...
    CREATE INDEX IF NOT EXISTS summary_messages_message_idx ON summary_messages (message_id);
  `);
    const conversationColumns = db2.prepare(`PRAGMA table_info(conversations)`).all();
    const hasBootstrappedAt = conversationColumns.some((col) => col.name === "bootstrapped_at");
    if (!hasBootstrappedAt) {
      db2.exec(`ALTER TABLE conversations ADD COLUMN bootstrapped_at TEXT`);
    }
    const hasSessionKey = conversationColumns.some((col) => col.name === "session_key");
    if (!hasSessionKey) {
      db2.exec(`ALTER TABLE conversations ADD COLUMN session_key TEXT`);
    }
    const hasActive = conversationColumns.some((col) => col.name === "active");
    if (!hasActive) {
      db2.exec(`ALTER TABLE conversations ADD COLUMN active INTEGER NOT NULL DEFAULT 1`);
    }
    const hasArchivedAt = conversationColumns.some((col) => col.name === "archived_at");
    if (!hasArchivedAt) {
      db2.exec(`ALTER TABLE conversations ADD COLUMN archived_at TEXT`);
    }
    db2.exec(`UPDATE conversations SET active = 1 WHERE active IS NULL`);
    db2.exec(`
    ...
    `);
    db2.exec(`DROP INDEX IF EXISTS conversations_session_key_idx`);
    runMigrationStep("ensureSummaryDepthColumn", log, () => ensureSummaryDepthColumn(db2));
    runMigrationStep(
      "ensureSummaryMetadataColumns",
      log,
      () => ensureSummaryMetadataColumns(db2)
    );
    runMigrationStep("ensureSummaryModelColumn", log, () => ensureSummaryModelColumn(db2));
    runMigrationStep(
      "ensureMessageIdentityHashColumn",
      log,
      () => ensureMessageIdentityHashColumn(db2)
    );
    runMigrationStep("ensureMessagePartsTable", log, () => ensureMessagePartsTable(db2));
    runMigrationStep(
      "backfillMessageIdentityHashes",
      log,
      () => backfillMessageIdentityHashes(db2, { managesOwnTransaction: false })
    );
    runMigrationStep(
      "createMessagesIdentityHashIndex",
      log,
      () => db2.exec(
    ...
    runMigrationStep(
      "ensureCompactionTelemetryColumns",
      log,
      () => ensureCompactionTelemetryColumns(db2)
    );
    runVersionedBackfillStep(db2, "backfillSummaryDepths", log, () => backfillSummaryDepths(db2));
    runMigrationStep(
      "createSummariesDepthIndex",
      log,
      () => db2.exec(
        `CREATE INDEX IF NOT EXISTS summaries_conv_depth_kind_idx ON summaries (conversation_id, depth, kind)`
      )
    );
    runVersionedBackfillStep(
      db2,
      "backfillSummaryMetadata",
      log,
      () => backfillSummaryMetadata(db2)
    );
    runVersionedBackfillStep(
      db2,
      "backfillToolCallColumns",
      log,
      () => backfillToolCallColumns(db2)
    );
    const detectedFeatures = options?.fts5Available === false ? null : getLcmDbFeatures(db2);
    ...
    if (fts5Available) {
      ...
      if (!trigramTokenizerAvailable) {
        try {
          db2.exec(`DROP TABLE IF EXISTS summaries_fts_cjk`);
        } catch {
        }
      }
      runMigrationStep("ensureMessagesFts", log, () => {
        ensureStandaloneFtsTable(db2, {
    ...
    runMigrationStep("ensureSummaryV41Columns", log, () => ensureSummaryV41Columns(db2));
    runMigrationStep(
      "ensureMessageSuppressedAtColumn",
      log,
      () => ensureMessageSuppressedAtColumn(db2)
    );
    runMigrationStep(
      "ensureLcmFeatureFlagsTable",
      log,
      () => ensureLcmFeatureFlagsTable(db2)
    );
    ...
    runMigrationStep("backfillForkRollupsSessionKeys", log, () => {
      const hasRollupsTable = db2.prepare(
        `SELECT name FROM sqlite_master WHERE type='table' AND name = 'lcm_rollups'`
      ).get();
      if (!hasRollupsTable) return;
      const rollupCols = db2.prepare(`PRAGMA table_info(lcm_rollups)`).all();
...
var MODEL_NAME_PATTERN = /^[A-Za-z0-9._-]{1,64}$/;
function embeddingsTableName(modelName) {
  if (!MODEL_NAME_PATTERN.test(modelName)) {
    throw new Error(
      `[embeddings.store] invalid model name: ${JSON.stringify(modelName)} (must match ${MODEL_NAME_PATTERN}; got len=${modelName.length})`
    );
  }
...
function tryLoadSqliteVec(db2, opts = {}) {
  const candidates = opts.path ? [opts.path] : candidateVec0Paths();
  for (const candidate of candidates) {
    if (!existsSync(candidate)) continue;
    try {
      db2.loadExtension(candidate);
      return true;
    } catch (e) {
      if (!opts.silent) {
        console.warn(
          `[embeddings.store] failed to load sqlite-vec at ${candidate}: ${e instanceof Error ? e.message : String(e)}`
        );
      }
    }
  }
...
function vec0Version(db2) {
  try {
    const row = db2.prepare("SELECT vec_version() AS v").get();
    return row?.v ?? null;
  } catch {
    return null;
  }
}
function ensureEmbeddingsTable(db2, modelName, dim) {
...
function registerEmbeddingProfile(db2, modelName, dim) {
  if (!MODEL_NAME_PATTERN.test(modelName)) {
  ...
  const row = db2.prepare(`SELECT dim FROM lcm_embedding_profile WHERE model_name = ?`).get(modelName);
  if (!row) {
    throw new Error(`[embeddings.store] failed to register profile ${modelName}`);
  }
  if (row.dim !== dim) {
    throw new Error(
      `[embeddings.store] dim mismatch for ${modelName}: existing profile has dim=${row.dim}, caller passed dim=${dim}. Profiles are immutable; bump model_name (e.g. add suffix) instead.`
    );
  }
}
function embeddingsTableExists(db2, modelName) {
  const tableName = embeddingsTableName(modelName);
  const row = db2.prepare(`SELECT name FROM sqlite_master WHERE type IN ('table', 'view') AND name = ?`).get(tableName);
  return Boolean(row?.name);
}

// src/voyage/client.ts
var MAX_TOKENS_PER_EMBED_DOC = 3e4;

// src/embeddings/backfill.ts
var DEFAULT_MIN_TOKEN_COUNT = 1;
function countPendingDocs(db2, args) {
  ...
  ).get(minTC, maxTC, kind, args.modelName);
  return row.n;
}

// sanity.mjs
var DB = "/Volumes/LEXAR/lcm-tmp/lcm-test-groupB-review.db";
var VEC0 = "/Users/lume/.openclaw/extensions/node_modules/sqlite-vec-darwin-arm64/vec0.dylib";
var db = new DatabaseSync(DB, { allowExtension: true });
console.log("[sanity] opened db");
var loaded = tryLoadSqliteVec(db, { path: VEC0 });
...
var t1 = Date.now();
var pending = countPendingDocs(db, { modelName: MODEL });
console.log("[sanity] pending leaves:", pending, "(took", Date.now() - t1, "ms)");
var totalLeaves = db.prepare(
  `SELECT COUNT(*) AS n FROM summaries WHERE kind='leaf' AND suppressed_at IS NULL`
).get();
var totalSummaries = db.prepare(`SELECT COUNT(*) AS n FROM summaries`).get();
var conv = db.prepare(`SELECT COUNT(*) AS n FROM conversations`).get();
console.log("[sanity] totals: leaves(unsuppressed)=", totalLeaves.n, "all summaries=", totalSummaries.n, "convs=", conv.n);
var tableMeta = db.prepare(`SELECT sql FROM sqlite_master WHERE name = ?`).get("lcm_embeddings_voyage4large");
console.log("[sanity] vec0 table SQL:", tableMeta?.sql ?? "(missing)");
var checkSampleId = db.prepare(`SELECT summary_id FROM summaries WHERE kind='leaf' LIMIT 1`).get();
console.log("[sanity] sample leaf:", checkSampleId?.summary_id);
var colInfo = db.prepare(`PRAGMA table_info(summaries)`).all().filter((c) => c.name === "session_key");
